...
STATE_W = 200   # less than Atari 160x192
STATE_H = 134
VIDEO_W = 600
VIDEO_H = 400
WINDOW_W = 600
...
class CarRacing(gym.Env, EzPickle):
    ...
    def _create_track(self):
        ...
        for i in range(len(track)):
            ...
            b2_l = (x2 + TRACK_WIDTH            * math.cos(beta2), y2 + TRACK_WIDTH            * math.sin(beta2))
            b2_r = (x2 + (TRACK_WIDTH + BORDER) * math.cos(beta2), y2 + (TRACK_WIDTH + BORDER) * math.sin(beta2))
            self.road_poly.append(( [b1_l, b1_r, b2_r, b2_l], (0, 0, 0) ))
            b3_l = (x1 - TRACK_WIDTH            * math.cos(beta1), y1 - TRACK_WIDTH            * math.sin(beta1))
            b3_r = (x1 - (TRACK_WIDTH + BORDER) * math.cos(beta1), y1 - (TRACK_WIDTH + BORDER) * math.sin(beta1))
            b4_l = (x2 - TRACK_WIDTH            * math.cos(beta2), y2 - TRACK_WIDTH            * math.sin(beta2))
            b4_r = (x2 - (TRACK_WIDTH + BORDER) * math.cos(beta2), y2 - (TRACK_WIDTH + BORDER) * math.sin(beta2))
            self.road_poly.append(( [b3_l, b3_r, b4_r, b4_l], (0, 0, 0) ))
        self.track = track
        return True
    ...
    def step(self, action):
        action2 = np.array(action, copy=True)
        vel = np.linalg.norm(self.car.hull.linearVelocity)
        if action is not None:
            if vel > 40:
                action2[1] = 0.0
            self.car.steer(-action2[0])
            self.car.gas(action2[1])
            self.car.brake(action2[2])

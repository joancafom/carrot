...
class CarrotRecorder(object):
    ...
    '''
        ...
        The distribution of the keys is the same as
        the ones found in many games:

        W - Forward (Gas)
        A - Left (Left)
        S - Return (Brake/Return)
        D - Right (Right) 

        Each time a button is pressed the action is performed, a new image
        of the road obtain along with the reward. That data is 
        then dumped to the summary CSV file.
        
        Checks every 500 ms if a button is being pressed
        or not to identify 'Centro-Gas' type of actions.
    '''
    def __init__(self):
    ...
    def write_new_frame(self, state, next_state, action, reward, done):
        ...
        self.csv_writer.writerow([action, reward, not done])
    ...
    def check_idle_loop(self):
        ...
        if self.last_key is 'None':
            ...
            self.write_new_frame(self.c_state, next_state, 1, reward, done)
